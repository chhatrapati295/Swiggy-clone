import { Drawer, Form, Input, Spin, notification } from "antd";
import { useEffect, useState } from "react";
import { useDispatch } from "react-redux";
import { setInitLocData, setLocData } from "../utils/slices/LocationSlice";
...
const Header = () => {
  ...
  const [initLoc, setInitLoc] = useState(true);
  ...
  const findRestaurants = async (placeId) => {
    const placeUrl = await fetch(
      `https://www.swiggy.com/dapi/misc/address-recommend?place_id=${placeId}`
    );
    const placeRes = await placeUrl.json();
    setIsDrawer(false);
    setInitLoc(false);
    localStorage.setItem('location_details' , JSON.stringify(placeRes?.data && placeRes?.data[0]?.geometry?.location) )
    dispatch(setLocData(placeRes?.data && placeRes?.data))
    setDrawerLoc(placeRes?.data && placeRes?.data[0]?.formatted_address);
  };

  // const getResList = async (resLocation) => {
  //   const resUrl = await fetch(
  //     `https://www.swiggy.com/dapi/restaurants/list/v5?lat=${resLocation?.geometry?.location?.lat}&lng=${resLocation?.geometry?.location?.lng}&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING`
  //   );
  //   const resData = await resUrl.json();
  //   console.log(resData);
  // };
  ...
  const getUserLocation = async () => {
    try {
      ...
      dispatch(setInitLocData(data))
    } catch {
      notification("Something wrong", "error", "top");
    }
  };
  ...
  return (
    <div className="py-4 px-12 border flex justify-between items-center shadow-md">
      <div className="flex gap-8 items-center">
        ...
        >
          ...
          <>
            {userLocation && initLoc ? (
              <span className="text-sm text-gray-400">
                {userLocation.city + " , " + userLocation.country_name}
                <i className="fas fa-chevron-down text-orange-500 ml-4"></i>
                {/* {userLocation} */}
              </span>
            ) : (
              <span className="text-sm text-gray-400">{drawerLoc} <i className="fas fa-chevron-down text-orange-500 ml-4"></i></span>
              // <Spin size="small"/>
            )}
          </>
        </div>
      </div>
      <ul className="flex gap-16 mr-6 capitalize  items-center font-medium  ">
        ...
        <li className="flex gap-2 items-center">
          ...
          <div className="relative">
            Offers{" "}
            <span className="absolute right-[-22px] top-[-6px] text-[11px] text-orange-500 font-semibold">
              New
            </span>
          </div>
        </li>
        ...
        <li className="flex gap-2 items-center">
          <svg
            className="fill-[#fff] stroke-2 stroke-[#000] overflow-hidden"
            viewBox="-1 0 37 32"
            height="22"
            width="22"
            fill="#686b78"
      ...
      >
        ...
        <ul className="">
          {searchData?.map((item, i) => {
            return (
              <li
                key={i}
                className="flex flex-col border-dotted border-b py-4 cursor-pointer"
                onClick={() => {
                  findRestaurants(item?.place_id);
                }}
              >
        ...
        {/* {!initLoc && <ul className="">
          <h2 className="">Recent Locations</h2>
          {recentLoc?.map((item, i)=>{
            return <li key={i} className="flex flex-col border-dotted border-b py-4 cursor-pointer" onClick={()=>{
              findRestaurants(item?.place_id)
            }}>
              <div className="flex gap-2 items-center">
                <i className="fa-solid fa-location-dot"></i>
